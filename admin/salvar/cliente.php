<?php
  //verificar se não está logado
  if ( !isset ( $_SESSION["tcc"]["id"] ) ){
    exit;
  }

  //verificar se existem dados no POST
  if ( $_POST ) {

  	//recuperar os dados do formulario
  	$id = $nome = $cpf = $nascimento = $email = $senha = $cep = $endereco = $bairro =  $msg = $telefone= $complemento=  "";

  	foreach ($_POST as $key => $value) {
  		//guardar as variaveis
  		$$key = trim ( $value );
  		//$id
  	}

  	//validar os campos - em branco
  	if ( empty ( $email ) ) {
  		echo '<script>alert("Preencha o email");history.back();</script>';
  		exit;
  	}


  	//verificar se existe um cadastro com este tipo
  	$sql = "select id from cliente 
  		where email = ? limit 1";
  	//usar o pdo / prepare para executar o sql
  	$consulta = $pdo->prepare($sql);
  	//passando o parametro
  	$consulta->bindParam(1, $email);
  	//executar o sql
  	$consulta->execute();
  	//puxar os dados (id)
  	$dados = $consulta->fetch(PDO::FETCH_OBJ);

  	//verificar se esta vazio, se tem algo é pq existe um registro com o mesmo nome
  	if ( !empty ( $dados->email ) ) {
  		echo '<script>alert("Já existe este email cadastrado");history.back();</script>';
  		exit;
  	}

  	//se o id estiver em branco - insert
  	//se o id estiver preenchido - update
  	if ( empty ( $id ) ) {
  		//inserir os dados no banco
  		$sql = "insert into cliente (nome,cpf,datanascimento,email,senha,cep,endereco,complemento,bairro,cidade_id,foto,telefone)
  		values(?,?,?,?,'senha',?,?,?,?,?,'foto',?)";
  		$consulta = $pdo->prepare($sql);
  		$consulta->bindParam(1, $nome);
        $consulta->bindParam(2, $cpf);
        $consulta->bindParam(3, $nascimento);
        $consulta->bindParam(4, $email);
        $consulta->bindParam(5, $cep);
        $consulta->bindParam(6, $endereco);
        $consulta->bindParam(7, $complemento);
        $consulta->bindParam(8, $bairro);
        $consulta->bindParam(9, $cidade);
        $consulta->bindParam(10, $telefone);
        


  	} else {
  		//atualizar os dados  	
  		$sql = "update cliente set marca = ? where id_marca = ? limit 1";	
  		$consulta = $pdo->prepare($sql);
  		$consulta->bindParam(1, $marca);
        $consulta->bindParam(2,$id);

  	}
  	//executar e verificar se deu certo
  	if ( $consulta->execute() ) {
  		echo '<script>alert("Registro Salvo");location.href="listar/cliente";</script>';
  	} else {
  		echo '<script>alert("Erro ao salvar");history.back();</script>';
  		exit;
  	}

  } else {
  	//mensagem de erro
  	//javascript - mensagem alert
  	//retornar hostory.back
  	echo '<script>alert("Erro ao realizar requisição");history.back();</script>';
  }